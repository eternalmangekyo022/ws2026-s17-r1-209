...
export default function Input({ labelText, textArea, id, rules: { isNum = false, length: [min, max] } }: Props) {
	const [input, setInput] = useState('');
	const [error, setError] = useState('');
	const { error: globalError, setError: setGlobalError, validate, setValidate } = useContext(RegisterContext)
	...
	async function validateInput(input: string): Promise<string> {
		if(input.length < min) return 'Input is shorter than expected'
		else if(!isNum && input.length > max) return 'Input is longer than expected'
		else if(isNum){
	...
	return textArea ? <div className="input-group">
		<label htmlFor="textarea">Description</label>
			<textarea className={error ? 'error': ''} id="textarea" value={input} onChange={handleOnChange} rows={5}></textarea>
		{error && <span className="input-error">{error}</span>}

  		</div>:
	<div className="input-group">
		<label htmlFor={`input-${id}`}>{labelText}</label>
		<input onChange={handleOnChange} value={input} type='text' id={`input-${id}`} className={error ? 'error': ''} />
		{error && <span className="input-error">{error}</span>}
    </div>
}